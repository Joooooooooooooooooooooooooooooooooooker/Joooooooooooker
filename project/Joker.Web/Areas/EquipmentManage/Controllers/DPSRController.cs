...
{
    public class DPSRController : ControllerBase
    {
        private DPSRApp dpsrApp = new DPSRApp();
        ...
        public ActionResult GetGridJson(Pagination pagination, string keyword)
        {
            var data = new
            {
                rows = dpsrApp.GetList(pagination, keyword),
        ...
        {
            var data = dpsrApp.GetForm(keyValue);
            return Content(data.ToJson());
        }
        [HttpPost]
        [HandlerAjaxOnly]
        [ValidateAntiForgeryToken]
        public ActionResult SubmitForm(DPSREntity entity, string keyValue)
        {
            dpsrApp.SubmitForm(entity, keyValue);
            return Success("操作成功。");
        }
        [HttpPost]
        [HandlerAuthorize]
        ...
        public ActionResult ExportExcel(string keyword)
        {
            ...
            dt.Columns.Add("参数编码");
            dt.Columns.Add("是否启用");
            dt.Columns.Add("是否在拓扑中显示");
            dt.Columns.Add("参数报警上限");
            dt.Columns.Add("参数报警下限");
            dt.Columns.Add("创建时间");
            dt.Columns.Add("创建用户");
            ...
            List<DPSREntity> dataList = dpsrApp.GetList();
            log.Info("获取报警信息--End");
            foreach (var item in dataList)
            {
                ...
                dr["是否启用"] = item.F_Data3;
                dr["是否在拓扑中显示"] = item.F_Data4;
                dr["参数报警上限"] = item.F_Data5;
                dr["参数报警下限"] = item.F_Data6;
            ...
            npoiexel.ToExcel(dt, "装置参数设置信息", "Sheet1", destDir + fileName);
