...
{
    public class EquipmentTopologyController : ControllerBase
    {
        private DTPIApp dtpiApp = new DTPIApp();
        private TPDIApp tpdiApp = new TPDIApp();
        [HttpPost]
        [HandlerAjaxOnly]
        public ActionResult GetDeviceToPoInfo() 
        {
            ...
            strResult = topoList.TopoViewToJson() + "|" + data2.ToJson();
            return Content(strResult);
        }

        [HttpPost]
        [HandlerAjaxOnly]
        public ActionResult GetToPoDataInfo()
        {
            var data = tpdiApp.GetList();
